...
def main():
    def menu0():
        ...
        if choice == "Perspectives_du_Cabinet_APEGG":
            ...
            ''')
            st.markdown(''' Environnement : Nous nous engageons à protéger et à restaurer l'environnement en proposant des solutions d'ingénierie environnementale intégrées, visant à réduire la pollution, à préserver les ressources naturelles et à promouvoir la durabilité         ''')
            ...
            st.markdown(''' Chez l'APEEG,nous croyons en un avenir ou la technologie et l'innovation sont mises au service de la durabilité et de la prospérité pour tous.Nous nous engageons à travailler en partenariat avec nos clients pour créer un impact positif sur la société et l'environnement,et à oeuvrer  ensemble pour une transition écologique globale reussie.''')
    ...
    def menu1():
        ...
        if choice=="Introduction":
            st.markdown('''### Conception d'un Cabinet de Conseil en Transformation Digitale et d'Ingénierie pour la Transition Écologique Globale!''') 
            ...
            url35="https://www.unfccc.int/sites/default/files/resource/inf04_0.pdf"
            st.markdown(f'<a href="{url35}" target="_blank">{"Cliquez ici pour visiter"+"www.mon éveil pour la Transition Ecologique Globale à Bonn.Allemagne_2017"}</a>', unsafe_allow_html=True)
            st.markdown('''###### L'héritage écologique du regretté Docteur Alain Pensé GAMASSA a été confié, selon ses voeux ,à l'un de ses frères. En 2017, son ONG, active dans la lutte contre les changements climatiques, a nommé Landry Didier GAMASSA comme représentant à la Conférence des Parties, qui s'est tenue du 6 au 17 novembre à Bonn, en Allemagne. Cette réunion, également connue sous le nom de Conférence de la Terre, a rassemblé 16 028 participants, dont des délégués d'ONG, des scientifiques, des décideurs économiques et des leaders politiques du monde entier, pour débattre des stratégies de lutte contre les changements climatiques. Les discussions ont mis en lumière des écarts par rapport aux engagements pris lors de la conférence sur le climat de Paris en 2017, visant à limiter le réchauffement de la planète à 1,5°C par rapport à l'ère préindustrielle, nécessitant ainsi des mesures contraignantes envers les pays les plus pollueurs. Organisée sous l'égide du PNUE (Programme des Nations Unies pour l’Environnement), cette conférence a souligné une fois de plus l'urgence d'agir face aux dommages causés à la planète par les activités humaines depuis la révolution industrielle. De 2017 à 2024, ce projet s'est consolidé grâce au temps, aux voyages, aux rencontres et aux expériences, bonnes ou mauvaises, qui ont contribué à établir ses fondations.
''' )
        ...
        if choice=="Environnement_d_un_Projet Smart Building":         
            ...
            if st.button("Enjeux de la reduction de la consommations énergétique en France"):
                ...
                st.text(">La thermosensibilité des Français. à -1°C en hiver =hausse de 3.2% de MW d'électricité")
            ...
            if st.button('''Tarif minimum estimatif d'un projet de suivi des consommations électriques avec flotte des capteurs Iot au Mètre Carré = 2euros/Mètres Carrés'''):
                st.write(''' Une première démarche peut etre réalisée sur un périmètre de 1000 Mètres carrés <=> Cout =2000€uros . Ensuite le projet peut se developper par itération ''')
...
''')            
    ...
    def menu7():
        ...
        if choice == "Environnement_de_Test":
            ...
            st.markdown(f'<a href="{url34}" target="_blank">{"Cliquez ici pour visiter"+"www.Catalogue_Capteur_Iot_Dataprint "}</a>', unsafe_allow_html=True)
    ...
    def menu9():
        ...
        if choice == "Industries":
            ...
            st.markdown(f'<a href="{url21}" target="_blank">{"Cliquez ici pour visiter"+"www.Iot2_et_Industrie"}</a>', unsafe_allow_html=True)     
            url23="https://youtu.be/DWhuPAR9N0A?si=Pgz79VXYP4S491hw"
            st.markdown(f'<a href="{url23}" target="_blank">{"Cliquez ici pour visiter"+"www.Iot3_et_Industrie"}</a>', unsafe_allow_html=True)    
    ...
    menu_selection = st.sidebar.radio("Sélectionnez une option",("0_Perspectives_du_Cabinet_APEGG","1_Etude de faisabilité","2_Fournisseurs_Réseau_LoRaWan","3_Fournisseurs_Instruments_IoT ","4_Budget","5_Cahier des Charges _SaaS","6_Architecture_Réseau","7_Environnement_Test|Production","8_Environnement_Deploiement","9_Et_Quel_Avenir_dans_5ans?"))

    # Afficher le contenu en fonction de la sélection du menu
    if menu_selection == "0_Perspectives_du_Cabinet_APEGG":
        menu0()
    elif menu_selection == "1_Etude de faisabilité":
        menu1()
